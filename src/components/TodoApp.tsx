...
export const TodoApp = () => {
  ...
  const updateStatusAllTodosOnServer = async () => {
    try {
      const processingTodos = activeTodos.length ? activeTodos : todos;

      await Promise.all(
        processingTodos.map((todo) =>
          updateStatusTodoOnServer(todo.id, todo.completed)
        )
      );
      await getTodosFromServer();
    } catch {
  ...
  const removeAllCompletedTodosFromServer = async () => {
    try {
      ...
      await Promise.all(
        processingTodos.map((todo) => removeTodoFromServer(todo.id))
      );
      await getTodosFromServer();
    } catch {
